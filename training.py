...
from model_definitions import VAE, Discriminator, combined_loss, WeightPredictor3DCNN, create_synthetic_video
...
# CNN Hyperparameters
CNN3D_IN_CHANNELS = CHANNELS
CNN3D_NUM_FILTERS = 32
...
transform = transforms.Compose([
    ...
    transforms.ToTensor(),
    transforms.Normalize((0.5, 0.5, 0.5), (0.5, 5, 0.5))  # Normalize to [-1, 1]
])
...
# --- Training Loop ---
def train_cnn3d_weight_predictor(device="cpu"):
    # --- Model Initialization ---
    vae = VAE().to(device)
    discriminator = Discriminator().to(device)
    weight_predictor = WeightPredictor3DCNN(CNN3D_IN_CHANNELS, CNN3D_NUM_FILTERS, WEIGHT_UPDATE_SIZE).to(device)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for idx, (real_images, _) in enumerate(loop):
            ...
            # 3. Get weight updates from 3D CNN
            weight_updates = weight_predictor(synthetic_video)

            # 4. Apply weight updates to VAE
            ...
            # 5. Generate images with *updated* VAE weights
            generated_images, mu, log_var = vae(real_images)

            # 6. Calculate generator loss
            loss_generator = -combined_loss(real_images, generated_images, discriminator, device)
            loss_generator.backward()

            # 7. Update VAE and weight predictor
            optimizer_vae.step()
    ...
    print(f"Training complete for 3D CNN Weight Predictor model")

# --- Main execution ---
if __name__ == "__main__":
    device = "cuda" if torch.cuda.is_available() else "cpu"
    train_cnn3d_weight_predictor(device)
