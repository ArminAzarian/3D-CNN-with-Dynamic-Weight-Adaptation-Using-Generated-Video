...
class VAE(nn.Module):  # Same VAE as before
    ...
    def forward(self, x):
        ...
        x_reconstructed = F.relu(self.fc3(x_encoded))
...
class WeightPredictor3DCNN(nn.Module):
    def __init__(self, in_channels, num_filters, output_size):
        super().__init__()
        self.cnn3d = nn.Sequential(
            nn.Conv3d(in_channels, num_filters, kernel_size=3, padding="same"),
            nn.ReLU(),
            nn.MaxPool3d(kernel_size=(2, 2, 2)),  # Reduce T, H, W

            nn.Conv3d(num_filters, num_filters * 2, kernel_size=3, padding="same"),
            nn.ReLU(),
            nn.MaxPool3d(kernel_size=(2, 2, 2))   # Reduce T, H, W
        )

        self.fc = nn.Linear(num_filters * 2 * 2 * 16 * 16, output_size) #Need to adapt the input to the video
        #This value are with the base parameters (num_frames=16, VIDEO_HEIGHT = 128, VIDEO_WIDTH = 128

    def forward(self, x):
        # x: (B, T, C, H, W) (Generated Video)
        x = x.transpose(1, 2) # from (B, T, C, H, W) to (B, C, T, H, W)

        x = self.cnn3d(x)
        x = torch.flatten(x, 1)
        weight_updates = self.fc(x)
